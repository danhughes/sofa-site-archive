...
window.RetweetJS = {
	...
	bitly_key: "R_c1995592419054f27143887a3afb876e",
...
function loaded(){
	...
	BitlyCB.shortenResponse = function(data) {
		for ( var url in data.results ) {
			...
			for ( var i = 0; i < elems.length; i++ ) {
				elems[i].href = elems[i].href.replace("bitly-id", hash);
			}
	...
	for ( var i = 0; i < elems.length; i++ ) {
		...
		elem.href = "http://twitter.com/home?status=" +
			encodeURIComponent(RetweetJS.prefix + "\"" + origText + "\"" + " http://bit.ly/bitly-id #madebysofa");
